...
class MemoryRepository(context: Context) : MemorySource {
    ...
    override fun memorySave(id: String, title: String, fromDate: String, toDate: String?, lat: Double, lon: Double, address:String, nation: String, text: String, uploadFile: File?, classification: Int, requestMemoryCallback: Callback?, activity: AppCompatActivity) {
        ...
        // 로컬 디비 저장
            (uploadFile == null) -> {
                memoryLocalDataSource.memorySave(id, title, fromDate, toDate, lat, lon, address, nation, text, uploadFile, classification, requestMemoryCallback, activity)
            }
        // 서버 디비 저장
            (uploadFile != null) -> {
                memoryRemoteDataSource.memorySave(id, title, fromDate, toDate, lat, lon, address, nation, text, uploadFile, classification, requestMemoryCallback, activity)
            }
        }
    }
